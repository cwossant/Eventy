...
HTML;
        ...
        if ($forceHtmlSingle) {
            // Send as single-part HTML (no plain-text alternative)
            $mail->isHTML(false); // do not build multipart automatically
            $mail->ContentType = 'text/html';
            $mail->Encoding = 'quoted-printable';
            $mail->AltBody = '';
            $mail->Body = $html;
        } else {
            // Standard multipart/alternative (HTML + plain text)
            $mail->isHTML(true);
            $mail->Encoding = 'quoted-printable';
            $mail->Body = $html;
            // Plain-text fallback
        ...
        // Log generated HTML body for inspection
        @file_put_contents($logFile, '[' . date('c') . "] HTML body size=" . strlen($html) . " bytes\n", FILE_APPEND);
    
        // In debug mode, log the MIME message PHPMailer will send
        $mimePreview = null;
        if ($debugMode) {
            // Build the message without sending to inspect headers and body
            if ($mail->preSend()) {
    ...
    // Optionally echo debug info to the page when debug=1
    if ($debugMode) {
    ...
    error_log($errLine);
    @file_put_contents(__DIR__ . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'mail.log', $errLine, FILE_APPEND);
